...
export default function CartPage() {
    ...
    // useEffect(() => {
    //   // if (userInfo?.user_id) {
    //     dispatch(fetchCart("5711b307-7f5d-4e98-a587-4a8306c2e676"));
    //   // }
    // }, [dispatch, userInfo]);

    useEffect(() => {
        if (userInfo?.user_id) {
            dispatch(fetchCart(userInfo.user_id));
        }
    }, [dispatch, userInfo]);

    const handleIncrement = (variant_id: string, currentQty: number) => {
        dispatch(updateCartItem({ user_id: "5711b307-7f5d-4e98-a587-4a8306c2e676", variant_id, quantity: currentQty + 1 }));
    };
    // const handleIncrement = (id: number) =>
    //     setProducts(prev => prev.map(p => p.id === id ? { ...p, qty: p.qty + 1 } : p));
    ...
    const handleToggleSelect = (variant_id: string, currentSelect: boolean) => {
        dispatch(updateCartItem({ user_id: "5711b307-7f5d-4e98-a587-4a8306c2e676", variant_id, is_selected: !currentSelect }));
    };

    const handleDelete = (variant_id: string) => {
        if (window.confirm("Hapus barang dari keranjang?")) {
            dispatch(deleteCartItem({ variant_id, user_id: "5711b307-7f5d-4e98-a587-4a8306c2e676" }));
        }
    };
    // const handleDelete = (id: number) =>
    //     setProducts(prev => prev.filter(p => p.id !== id));
    ...
    const allSelected =
      items.length > 0 &&
      items.every(item => Boolean(item.is_selected));
    ...
    const handleSelectAll = async () => {
        ...
        await Promise.all(
            ...
            dispatch(updateCartItem({
                user_id: "5711b307-7f5d-4e98-a587-4a8306c2e676",
