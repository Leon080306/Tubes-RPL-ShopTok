          ...
          {[
            ...
            <Grid size={6} key={m.label}>
              <Box sx={{ textAlign: "center", p: 1.25, bgcolor: "#f8fafc", borderRadius: 2 }}>
                <Typography sx={{ fontSize: 18, fontWeight: 700, color: "#0d1f13" }}>{m.value}</Typography>
                <Typography sx={{ fontSize: 11.5, color: "#94a3b8" }}>{m.label}</Typography>
              </Box>
            </Grid>
          ))}
        </Grid>
    ...
    <ThemeProvider theme={theme}>
      <Box sx={{ display: "flex", height: "100vh", bgcolor: BG, overflow: "hidden" }}>
        <Sidebar
          ...
          shop={shop}
          onSelect={(id) => {
            if (id === "messages") navigate("/chattoko");
