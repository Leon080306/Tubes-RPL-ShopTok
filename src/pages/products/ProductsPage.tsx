...
function adaptProduct(p: BackendProduct): ProductUI {
    const variants = p.variants ?? [];
    const ratings = p.ratings ?? [];

    const prices = variants.map((v) => Number(v.price)).filter((x) => x > 0);
    const minPrice = prices.length > 0 ? Math.min(...prices) : 0;

    const avgRating =
        ratings.length > 0
            ? ratings.reduce((acc, r) => acc + Number(r.value), 0) / ratings.length
            : 0;
...
export default function ProductsPage() {
    ...
    useEffect(() => {
        const getCategories = async () => {
            try {
                ...
                const fetched: Category[] = data.records || [];
                setCategories(fetched);

                // If URL has a category that doesn't exist in DB, clear it
                const catFromUrl = searchParams.get("category") ?? "";
                if (catFromUrl && !fetched.some((c) => c.name === catFromUrl)) {
                    setSelectedCategory("");
                }
            } catch (error) {
    ...
    /* ================= ADD TO CART ================= */
    const handleAddToCart = async (e: React.MouseEvent, product: ProductUI) => {
        e.stopPropagation();
        ...
        if (product.stock === 0 || !product.firstVariantId) {
    ...
    const activeFilters = [
        ...
        sortPrice && {
            label: sortPrice === "high" ? "Price: High" : "Price: Low",
            clear: () => setSortPrice(""),
        },
        sortRating && {
            label: sortRating === "high" ? "Rating: High" : "Rating: Low",
            clear: () => setSortRating(""),
        },
        minRating > 0 && {
            label: `Min ${minRating}★`,
            ...
            >
                ...
                <FormControl size="small" sx={{ width: 150 }}>
                    <Select
                        ...
                        renderValue={(v) =>
                            v || <span style={{ opacity: 0.5 }}>Category</span>
                        }
                    >
                ...
                <FormControl size="small" sx={{ width: 130 }}>
                    <Select
                        ...
                        renderValue={(v) =>
                            !v ? (
                                <span style={{ opacity: 0.5 }}>Rating</span>
                            ) : v === "high" ? (
                                "Top Rated"
                            ) : (
                                "Lowest Rated"
                ...
                <FormControl size="small" sx={{ width: 130 }}>
                    <Select
                        ...
                        renderValue={(v) =>
                            !v ? (
                                <span style={{ opacity: 0.5 }}>Min Rating</span>
                            ) : (
                                `Min ${v}★`
                            )
                        }
